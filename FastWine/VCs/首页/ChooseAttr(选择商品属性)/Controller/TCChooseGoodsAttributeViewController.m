...
- (void)fetchGoodsAttributesData {
    ...
    self.attrModel = [TCAttributeModel mj_objectWithKeyValues:_dataDic[@"data"]];
    
    self.dataSource = _dataDic[@"data"][@"productAttr"];
//    NSArray *sortDescriptors = [NSArray arrayWithObject:[NSSortDescriptor sortDescriptorWithKey:@"attr_name" ascending:YES]];
//    [self.dataSource sortedArrayUsingDescriptors:sortDescriptors];
//    NSArray *result = [self.dataSource sortedArrayUsingSelector:@selector(localizedCompare:)];
//    NSLog(@"排序后的数组%@",result);
//
//    NSLog(@"localizedCompare:%@",self.dataSource);

    NSMutableArray *noteSpecArr = @[].mutableCopy;
    for (NSDictionary *dic in self.dataSource) {// 组合选择提示语
        [noteSpecArr addObject:dic[@"attr_name"]];
    }
    self.noteSpec = [noteSpecArr componentsJoinedByString:@" "];
    NSMutableArray *skuArray = @[].mutableCopy;
    for (Attribute *model in self.attrModel.productValues) {
        NSMutableDictionary *dic = [NSMutableDictionary dictionaryWithCapacity:1];
        NSArray *array = [model.suk componentsSeparatedByString:@","];
//        NSArray *result = [array sortedArrayUsingSelector:@selector(localizedCompare:)];
        //数组倒序
         NSArray *result = (NSMutableArray *)[[array reverseObjectEnumerator] allObjects];
        NSMutableString * newsuk = [[NSMutableString alloc]init];
        [result enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
             [newsuk appendFormat:@",%@", obj];
        }];
        
        dic[@"contition"] = [newsuk substringFromIndex:1];
        NSLog(@"localizedCompare:%@------newsuk:%@",result,[newsuk substringFromIndex:1]);
...
//    //默认第一个
//    Attribute *attribute =  self.attrModel.productValues[self.attrModel.productValues.count - 1];
...
- (void)setupSubViews {
    ...
    ViewBorderRadius(_goodImageView, 5, 5, WhiteColor);
...
//    _lb_price.text = @"￥00.00";
    if ([ACCOUNTINFO.userInfo.is_promoter integerValue]  == 1) {
        _lb_price.text = [NSString stringWithFormat:@"￥%@", _choosedAttribute.vip_price];
    } else {
        _lb_price.text = [NSString stringWithFormat:@"%@", _choosedAttribute.price];
    }
    ...
    _lb_stock.text = _choosedAttribute.stock;
    ...
    _lb_detail.text = [NSString stringWithFormat:@"已选择 %@",_choosedAttribute.suk];
    
    [self.view addSubview:_lb_detail];
    
    _closeBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    [_closeBtn setBackgroundImage:[UIImage imageNamed:@"cuo"] forState:UIControlStateNormal];
...
//    [self.view addSubview:_addCartBtn];
    ...
    [_buyGoodsBtn setTitle:@"确认" forState:UIControlStateNormal];
    ...
    [_goodImageView sd_setImageWithURL:[NSURL URLWithString: _choosedAttribute.image] placeholderImage:[UIImage imageNamed:Goods_noImage] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
    ...
    _quantity = @"1";
    _maxQuantity = [_choosedAttribute.stock integerValue];
    _lb_stock.text = [NSString stringWithFormat:@"库存%@件", _choosedAttribute.stock];
    if ([ACCOUNTINFO.userInfo.is_promoter integerValue]  == 1) {
        _lb_price.text = [NSString stringWithFormat:@"￥%@", _choosedAttribute.vip_price];
    } else {
        _lb_price.text = [NSString stringWithFormat:@"%@", _choosedAttribute.price];
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section {
    ...
    return [_dataSource[section][@"attr_values"] count];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSArray *data = _dataSource[indexPath.section][@"attr_values"];
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([kind isEqualToString:UICollectionElementKindSectionHeader]) {
        ...
        view.headernameL.text = _dataSource[indexPath.section][@"attr_name"];
        return view;
    } else {
...
- (NSArray *)filter:(ORSKUDataFilter *)filter propertiesInSection:(NSInteger)section {
    ...
    return _dataSource[section][@"attr_values"];
}
...
//        _choosedAttribute.info = nil;
    ...
    if ([ACCOUNTINFO.userInfo.is_promoter integerValue]  == 1) {
        _lb_price.text = [NSString stringWithFormat:@"%@", attribute.vip_price];
    } else {
        _lb_price.text = [NSString stringWithFormat:@"￥%@", attribute.price];
    }
    _lb_stock.text = [NSString stringWithFormat:@"库存%@件",attribute.stock];
    _lb_detail.text = [NSString stringWithFormat:@"已选择 %@", attribute.suk];
    
    __weak typeof(self) _weakSelf = self;
    [_goodImageView sd_setImageWithURL:[NSURL URLWithString: attribute.image] placeholderImage:ZHENG_HOLDER_IMAGE completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
        if (image && cacheType == SDImageCacheTypeNone) {
            _weakSelf.goodImageView.alpha = 0;
            [UIView animateWithDuration:1.0 animations:^{
                _weakSelf.goodImageView.alpha = 1.0f;
...
- (void)finishChooseAttribute {
    ...
    _spec_id = _choosedAttribute.unique;

    _specification = _choosedAttribute.suk;
}
- (void)animateCartAdd {
    __weak typeof(self) _weakSelf = self;
...
- (void)buyNowBtnClick {
    ...
    if (_goBuyBlok) {
        _goBuyBlok(self,_choosedAttribute.unique,[self.quantity integerValue]);
    }
}
