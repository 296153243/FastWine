...
- (void)setInfoArr:(NSArray *)infoArr
{
    ...
    self.bgScrollView.contentSize = CGSizeMake(self.qu_w, infoArr.count*self.qu_h);
...
- (void)setLoopView
{
    for (int i = 0; i<self.infoArr.count+2; i++) {
        ...
        cellBtn.frame = CGRectMake(0, self.qu_h*i,self.qu_w, self.qu_h);
        cellBtn.tag = 100+i;
        ...
        UIImageView * imgV = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, cellBtn.qu_h, cellBtn.qu_h)];
        imgV.layer.cornerRadius = cellBtn.qu_h/2;
        imgV.layer.masksToBounds = YES;
        [cellBtn addSubview:imgV];

        UILabel * label = [[UILabel alloc]initWithFrame:CGRectMake(imgV.qu_x, 0, cellBtn.qu_w-imgV.qu_w, cellBtn.qu_h)];
...
- (void)cellScroTimerFired:(NSTimer *)timer
{
    if (self.cellScroTimer_yu) {
        ...
        if (self.cellScroTimer_yu_jsh == 100*_keepSecond) {
            ...
            if (self.bgScrollView.contentOffset.y >= self.infoArr.count*self.qu_h) {
        ...
        if (self.cellScroTimer_yu_jsh == 100) {
            ...
            self.bgScrollView.contentOffset = CGPointMake(0, self.bgScrollView.contentOffset.y+self.qu_h/100);
        }
    }
...
- (UIScrollView *)bgScrollView
{
    if (_bgScrollView == nil) {
        _bgScrollView = [[UIScrollView alloc]initWithFrame:CGRectMake(0, 0, self.qu_w, self.qu_h)];
