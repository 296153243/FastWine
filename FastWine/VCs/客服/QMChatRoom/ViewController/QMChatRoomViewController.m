...
@interface QMChatRoomViewController ()<UITableViewDataSource, UITableViewDelegate, UITextViewDelegate, TCMessageManagerFaceViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UIAlertViewDelegate, AVAudioRecorderDelegate, QMKServiceDelegate, QMAudioRecorderDelegate> {
    ...
    NSString *intelligentRobot; //智能机器人id
...
- (void)createUI {
    ...
    // 消息列表
    self.chatTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT-kInputViewHeight-_navHeight) style:UITableViewStylePlain];
    ...
    // 输入工具条
    self.chatInputView = [[QMChatRoomInputView alloc] initWithFrame:CGRectMake(0, SCREEN_HEIGHT-kInputViewHeight-_navHeight, SCREEN_WIDTH, kInputViewHeight)];
    ...
    // 表情面板
    self.faceView = [[TCMessageManagerFaceView alloc] initWithFrame:CGRectMake(0, [UIScreen mainScreen].bounds.size.height, SCREEN_WIDTH, IS_iPhoneX ? 250 : 216)];
    self.faceView.delegate = self;
    [self.faceView.sendButton addTarget:self action:@selector(sendBtnAction:) forControlEvents:UIControlEventTouchUpInside];
    
    // 扩展面板
    self.addView = [[QMChatRoomMoreView alloc] initWithFrame:CGRectMake(0, [UIScreen mainScreen].bounds.size.height, SCREEN_WIDTH, IS_iPhoneX ? 144 : 110)];
    ...
    self.addView.evaluateBtn.hidden = true;
    self.addView.evaluateLabel.hidden = true;
    ...
    [self.manualButotn setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
...
//    [self.logoutButton setTitle:NSLocalizedString(@"button.logout", nil) forState:UIControlStateNormal];
    ...
    self.navigationItem.leftBarButtonItem = [UIBarButtonItem itemWithTarget:self action:@selector(logoutAction) image:@"Back_icon"];
    
    
    self.indicatorView = [[QMRecordIndicatorView alloc] init];
    self.indicatorView.frame = CGRectMake((SCREEN_WIDTH-150)/2, (SCREEN_HEIGHT-150-_navHeight-50)/2, 150, 150);
...
- (void)evaluateBtnAction {
    ...
    if (_isRobot) {
        ...
        UIAlertAction * resolvedAction = [UIAlertAction actionWithTitle:@"已解决" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
            [self pushSatisfaction:@"true" robotId:intelligentRobot];
        }];
        ...
        UIAlertAction * unsolvedAction = [UIAlertAction actionWithTitle:@"未解决" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
            [self pushSatisfaction:@"false" robotId:intelligentRobot];
        }];
...
- (void)logoutAction {
    ...
    for (UIViewController *vc in self.navigationController.viewControllers) {
        ...
        if ([vc isKindOfClass:[OrderDetailsVC class]]) {
            [self.navigationController popToViewController:vc animated:YES];
            return ;
        }
...
- (void)showGuestBookViewController {
    ...
    self.chatTableView.frame = CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT - _navHeight);
...
- (void)keyboardFrameChange: (NSNotification *)notification {
    ...
    if (newFrame.origin.y == [UIScreen mainScreen].bounds.size.height) {
        [UIView animateWithDuration:0.3 animations:^{
            self.chatInputView.frame = CGRectMake(0, SCREEN_HEIGHT-kInputViewHeight-_navHeight, SCREEN_WIDTH, kInputViewHeight);
            self.chatTableView.frame = CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT-_navHeight-kInputViewHeight);
        }];
    }else {
        [UIView animateWithDuration:0.3 animations:^{
            if (!keyboardIsShow) {
                self.chatInputView.frame = CGRectMake(0, [UIScreen mainScreen].bounds.size.height-kInputViewHeight-newFrame.size.height-_navHeight, SCREEN_WIDTH, kInputViewHeight);
                self.chatTableView.frame = CGRectMake(0, 0, SCREEN_WIDTH, [UIScreen mainScreen].bounds.size.height-_navHeight-kInputViewHeight-newFrame.size.height);
                [self scrollToEnd];
            }
        }];
    }
...
- (void)customSatisfaction:(NSNotification *)notification {
    ...
    if (arr[1]) {
        intelligentRobot = arr[1];
    }
    
    if ([arr[0] isEqualToString:@"true"]) {
        isShowEvaluate = true;
        if (_isSpeak) {
            self.addView.evaluateBtn.hidden = false;
            self.addView.evaluateLabel.hidden = false;
        }
    }
