...
- (void)createUI {
    CGFloat height = [self calcLabelHeight:self.headerTitle ?: NSLocalizedString(@"title.messageHeader", nil) font:[UIFont systemFontOfSize:17] width:SCREEN_WIDTH - 20];
    CGFloat labelHeight = height > 25 ? height : 25;
    
    self.headerView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, 185+labelHeight)];
    self.headerView.backgroundColor = [UIColor colorWithRed:244/255.0 green:244/255.0 blue:246/255.0 alpha:1];
    
    self.henaderLabel = [[UILabel alloc] initWithFrame:CGRectMake(10, 10, SCREEN_WIDTH, labelHeight)];
    ...
    self.textView = [[UITextView alloc] initWithFrame:CGRectMake(10, CGRectGetMaxY(self.henaderLabel.frame)+10, SCREEN_WIDTH-20, 165)];
    ...
    self.submitBtn.frame = CGRectMake(20, 30, SCREEN_WIDTH-40, 44);
...
- (void)keyboardFrameChange: (NSNotification *)notification {
    ...
    if (newFrame.origin.y == [UIScreen mainScreen].bounds.size.height) {
        ...
        [UIView animateWithDuration:0.3 animations:^{
            _tableView.frame = CGRectMake(0, 0, SCREEN_WIDTH, [UIScreen mainScreen].bounds.size.height-64-newFrame.size.height);
        }];
    }
}
