...
//        _backgroundBtn.alpha = self.backMaskAlpha;
        _backgroundBtn.backgroundColor  =[UIColor colorWithWhite:0.2 alpha:0.2];
...
#pragma mark - 加载地址数据
- (void)loadAddressJSONData{
    NSArray *array  =[self JsonObject:@"area.json"];
    NSArray *citysModelArrs = [AddressProvince mj_objectArrayWithKeyValuesArray:array];
    self.provincesArr = [NSMutableArray array];
   __block NSInteger markid;
    NSMutableArray * areasArr = [[NSMutableArray alloc]init];
     NSMutableArray * quArr = [[NSMutableArray alloc]init];


    [citysModelArrs enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        
       AddressProvince *model = obj;
        if (model.pid == 0) {
            self.dataModelArr =obj;

        }
        if (model.pid == self.dataModelArr.id) {
         
               [areasArr addObject:model];

        }
     
        self.dataModelArr.cityModels = areasArr;
...
- (NSString *)pickerView:(UIPickerView *)pickerView
    ...
    ((UILabel *)[pickerView.subviews objectAtIndex:1]).backgroundColor = LineColor;//显示分隔线
    ((UILabel *)[pickerView.subviews objectAtIndex:2]).backgroundColor = LineColor;
    if (0 == component) {
...
- (void)sureBtnClicked{
    ...
    if ([_delegate respondsToSelector:@selector(sureBtnClickReturnProvince:City:Area:cityCode:)]) {
     ...
     //根据选择的城市 取code
        NSArray *array  =[self JsonObject:@"area.json"];
        NSArray *citysModelArrs = [AddressProvince mj_objectArrayWithKeyValuesArray:array];
        _citysModelArrs = [NSMutableArray array];
     __block   AddressProvince *chengshi ;
        [citysModelArrs enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            AddressProvince *model = obj;
            
            if ([c.cityName containsString:model.name] || [c.cityName isEqualToString:model.name]) {
                chengshi = model;
                
            }
          
            if (chengshi.id == model.pid) {
                //将城市的地级市加入到数组里
                [self.citysModelArrs addObject:model];
            }
            
            
        }];
        
        [self.citysModelArrs enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
             AddressProvince *model = obj;
            if ([c.areas[selectArea] containsString:model.name] || [c.areas[selectArea] isEqualToString:model.name]) {
                self.dataModelArr = model;
        ...
        [_delegate sureBtnClickReturnProvince:p.name
                                         City:c.cityName
                                         Area:c.areas[selectArea] cityCode:_dataModelArr.id];
